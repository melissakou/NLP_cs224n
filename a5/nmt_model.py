...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device) # Tensor: (src_len, b)
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device) # Tensor: (src_len, b, max_word_length)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device) # Tensor: (src_len, b, max_word_length)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        packed_X = nn.utils.rnn.pack_padded_sequence(input=X, lengths=source_lengths)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            dec_state, o_t, _ = self.step(Ybar_t=Ybar_t, dec_state=dec_state, enc_hiddens=enc_hiddens, enc_hiddens_proj=enc_hiddens_proj, enc_masks=enc_masks)
