...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        num_batches = len(unfinished_parses) // batch_size + 1
        for i in range(num_batches):
            parses_batch = unfinished_parses[i*batch_size:(i+1)*batch_size]
            transitions_batch = model.predict(parses_batch)
            [parse.parse_step(transition) for (parse, transition) in zip(parses_batch, transitions_batch)]
